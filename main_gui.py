...
class NetworkProjectGUI:
    ...
    def setup_ui(self):
        ...
        ttk.Radiobutton(pnl_algo, text="Genetik Algoritma (GA)", variable=self.algo_var, value="GA",
                        bootstyle="warning-toolbutton").pack(fill=X, pady=2)
        ttk.Radiobutton(pnl_algo, text="Pekiştirmeli Öğrenme (RL)", variable=self.algo_var, value="QL",
                        bootstyle="warning-toolbutton").pack(fill=X, pady=2)
        ...
        self.lbl_guvenilirlik = ttk.Label(pnl_result, text="Güvenilirlik Oranı: -", font=("Helvetica", 10),
                                          bootstyle="success")
        self.lbl_guvenilirlik.pack(anchor=W, pady=2)

        self.lbl_kaynak = ttk.Label(pnl_result, text="Ağ Kaynak Kullanımı: -", font=("Helvetica", 10),
                                    bootstyle="primary")
        self.lbl_kaynak.pack(anchor=W, pady=2)

        self.lbl_cost = ttk.Label(pnl_result, text="Durum: Bekleniyor...", font=("Helvetica", 9, "bold"),
                                  bootstyle="danger")
        self.lbl_cost.pack(anchor=W, pady=(5, 0))

        self.btn_run = ttk.Button(control_panel, text="⚡ OPTİMİZASYONU BAŞLAT", command=self.run_optimization,
    ...
    def draw_graph(self, path=None):
        ...
        if path:
            ...
            nx.draw_networkx_edges(self.G, self.pos, ax=self.ax, edgelist=path_edges, edge_color='#00f2ff', width=6,
                                   alpha=0.4)
            ...
            nx.draw_networkx_nodes(self.G, self.pos, ax=self.ax, nodelist=[path[0]], node_size=150,
                                   node_color='#1dd1a1', label="Start")
            nx.draw_networkx_nodes(self.G, self.pos, ax=self.ax, nodelist=[path[-1]], node_size=150,
                                   node_color='#ff6b6b', label="End")
