...
def compute_reward(G, u, v):
    ...
    reliability = -math.log(r_link) - math.log(
        r_node)  # reliabilityCost(P)=X(i,j)∈P[−log( RLinkReliabilityij)]+Xk∈P[−log(N odeReliabilityk)]
...
def train_q_learning(G, demands):
    ...
    for episode in range(EPISODES):
        ...
        for _ in range(MAX_STEPS):
            neighbors = feasible_neighbors(G, current,
                                           demand_bw)  # eğer bu node'dan uygun çıkış yoksa episode burada bitiyor
            if not neighbors:
                break
